...
$crud->setListingQuery("SELECT * FROM HR_Volunteer_Request ORDER BY added_on DESC ");
...
function getRecCount($row) {
	...
	$num = $sql->getOne("SELECT COUNT(U.id) FROM User U
				...
				// AND U.joined_on <= '" . $all_cycles_dates[$row['cycle']]['end'] . " 00:00:00'"); // This would make sure only the volunters who joined in that cicle will show up - but now, show all.
	return $num;
}